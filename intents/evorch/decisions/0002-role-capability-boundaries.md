...
- tripwire: web_search_network_gate.rs / web_fetch_network_gate.rs を非対称公開へ更新 (Librarian=両方許可 / Orchestrator=fetch のみ OptIn / Explorer・Worker・Reviewer=拒否)。Orchestrator の web_fetch は `judge_web_network_access` 3 層 AND で session NetworkAccess=OptIn の approval (Ask) を経由する。
...
- 未配線 (後続 slice): session NetworkAccess の RunConfig 化と `judge_web_network_access` の agent loop 配線 (production executor の ApprovalPolicy は allow_all のまま — GUI production では approval prompt なしという既知ギャップ)、Librarian の config フィールド / prompt baseline / `parse_role` / GUI 露出。
