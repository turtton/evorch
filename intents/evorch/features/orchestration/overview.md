...
- **config schema**（2026-09-16 更新）: `[agents]` セクションの非workerロールは `RoleBindingConfig{logical_model,preset,generation{temperature,top_p,max_tokens,reasoning_effort}}`、worker のみ `WorkerBindingConfig{base: RoleBindingConfig（serde flatten）,categories}`。カテゴリは worker 専用の固定 6 種（quick / deep / high-reasoning / visual / writing / research）で検証し、非worker（`agents.roles` 配下を含む）の categories は strict 検証と serde で拒否する。`binding_for(role,category)` も非workerへのカテゴリ指定を拒否し、worker では per-field category-beats-role マージで `ResolvedAgentBinding` を返す。config version は 2 のまま。
