...
web_fetch tool の確定実装（issue #45、PR #46、2026-09-02）: `crates/tools/src/tools/web_fetch/` の builtin tool として上記 q01・q04-q07・q10 が実装で確定。extracter チェーンは `Extractor` trait + `ExtractionChain::standard(selector)`（明示 selector → dom_smoothie 0.18 readability（`is_probably_readable` 前置）→ full document fallback）で、先頭に site-aware extractor を差し込む拡張点 `ExtractionChain::with_prepended` を持つ。採用 crate: scraper 0.27 / dom_smoothie 0.18 / htmd 0.5.5（dom_smoothie は readability 系として検証のうえ採用）。size pipeline は NetworkGuard の 5MB 三面制限（Content-Length 事前 / streaming 累計 / 解凍後累計）+ model-facing 50KB UTF-8 安全 byte-prefix 切詰め（`truncate.rs`）で metadata（truncated / original_bytes / redirect_count / redirect_blocked / extraction_method 等）は ToolCompleted detail に包含。NetworkGuard 拡張: `GuardedResponse.final_url` / `redirect_count` 公開、redirect 先（hop ≥ 1）の遮断 IP を `NetworkGuardError::RedirectBlocked` に分類（初回 hop は従来どおり `BlockedIp`）。権限 3 層 AND と `ContentOrigin::WebUntrusted` 機械導出は既存機構を流用し、production layer-1 gate の web_fetch 遮断は `crates/runtime/tests/web_fetch_network_gate.rs` で tripwire 固定。
