...
**v0.2 進捗（2026-09-03、issue #55）**: OTel metrics exporter の slice ①（写像層＋OTLP metrics exporter）が landed。`crates/event-bus` に `event_bus::otel` module（写像層は常時 compile、SDK 接続は opt-in feature `otel-exporter`、既定 off）を追加し、`gen_ai.*` 標準属性＋`evorch.*` 拡張への写像・属性 whitelist 8 key の cardinality guard・golden fixture 検証・InMemory＋実 OTLP HTTP の二重 reader による最小 E2E を実装（詳細は ADR 0023 最終版）。`CacheStats`（hit/miss 系）は v0.3 以降送りを維持。残る v0.2 gap は slice ②（span exporter）のみとなった（後続の issue #57 で landed。次項）。

**v0.2 進捗（2026-09-03、issue #57）**: OTel span exporter の slice ②（写像層の span 化拡張＋OTLP span exporter）が landed。`crates/event-bus` の `event_bus::otel::span` に stateful span mapper（feature 非依存、opentelemetry 非参照）を追加し、内部 event から session / run / agent(`invoke_agent`) / request(`chat`) / tool(`execute_tool`) の 5 層 span tree を構築する（GenAI spans v1.37.0 pin）。run 相関のため `LifecycleEvent::AgentRunStarted` と provider / tool event の `run_id` フィールドを additive に schema 追加（schema_version=1 維持）し、runtime / providers / tools の boundary context（AgentInvocationContext / ObservationContext / ToolExecutionContext）で threading した（ObservationContext は wire 非搭載）。span attribute はソート済み whitelist 18 key の閉集合＋key 別閉 domain 検査＋raw-content denylist で規律化し、ID 系は span 層限定（metrics whitelist との非交差を guard で保証）。run sampling（決定的 FNV-1a、ratio 既定 1.0、子相続）と ADR 0012 hard limits（per-run in-flight 128／global 4096／window 10_000・60s／属性 32 件・16KiB・値 1KiB／lifetime 30min eviction／tombstone 4096）を admission 一箇所に実装し、写像不能事象は typed drop 10 種として記録する。SDK 接続層は feature `otel-exporter` の `OtelSpanEmitter`＋OTLP / InMemory tracer provider 構築関数（`/v1/traces` 手動 normalize、`SimpleSpanProcessor`＋`Sampler::AlwaysOn`、新規依存なし）。検証は手書き golden fixture 10 本＋cardinality guard span 5 本追加＋InMemory／loopback OTLP HTTP（POST `/v1/traces` 疎通 assert）の二重 reader span E2E＋runtime integration（実 provider wire 経由の run_id 相関）で、issue の AC1-7（委譲親子 = span 親子写像／semconv pin 明記／ID の span 保持と metrics 非載／raw 非含有／sampling＋hard limits 実効／golden＋CI cardinality guard 拡張／最小 OTLP E2E 1 本）を充足した。subscription wiring（EventBus→mapper→emitter の常駐配線）は ADR 0014 の config 有効化経路に属するため後続 slice に defer。これにより v0.2 seed の OTel export 面（metrics＋span）は揃った。
