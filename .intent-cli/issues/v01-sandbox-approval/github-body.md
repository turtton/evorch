...
- Linux sandbox 選択は tools-sandbox overview の Open question のため、本 packet 冒頭で選定し ADR 0017 化（推奨: bwrap）。bwrap は user namespaces + network namespace で egress deny を実現でき、Landlock は filesystem のみで network 制御不可
...
- Linux sandbox 第一実装の選択と ADR 0017 化（最初のタスク。推奨: bwrap。bwrap 実行不可環境では fail-closed）
...
- Landlock vs bwrap の選択結果が ADR 0017 として記録されている
...
- `intents/evorch/decisions/0017-sandbox-linux-bwrap.md` の存在と内容を closeout で確認
...
- ADR candidate: ADR 0017（Linux v0.1 sandbox 第一実装: bubblewrap（bwrap）採用）
- Diagram candidate: none
- Docs update: none（role-facing surface を追加しないため）
- Closeout writeback expected: yes（ADR 0017 + tools-sandbox overview の Open question 解消）
