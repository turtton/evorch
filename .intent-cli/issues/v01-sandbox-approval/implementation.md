...
- **Linux sandbox 第一実装の選択（最初のタスク）**: Landlock vs bwrap の比較検討と ADR 0021 化。bwrap（bubblewrap）を推奨（user namespaces + network namespace で egress deny を実現でき、ADR 0008 の「network egress 既定 deny」に対応しやすい。Landlock は filesystem アクセス制御のみで network 制御ができない）。bwrap が実行不可の環境（非 root / kernel 制限）では fail-closed とする方針も ADR で記録
...
- ADR 0021 の存在（`intents/evorch/decisions/0021-sandbox-linux-bwrap.md`）を closeout で確認
...
- ADR candidate: **あり** — Linux sandbox 第一実装の選択（Landlock vs bwrap）は後戻りしにくい決定で、本 packet 冒頭の必須タスクとして ADR 0021 に記録する
...
- Closeout learning: ADR 0021 の新設と tools-sandbox overview の Open question 解消を write back する。`write_back_required: true`
