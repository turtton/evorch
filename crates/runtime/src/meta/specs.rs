    ...
    match name {
        ...
        "wait" => {
            let mut spec = object_spec(name,
                "Wait for terminal completion of directly related parent/child runs using runtime notifications, without consuming their inboxes. Prefer one bounded wait over frequent status polling; continue independent work before waiting. Supply exactly one of run_id or run_ids (1-8 unique IDs); mode=any returns when one finishes, mode=all when all finish. Either mode returns early when a target needs a required user answer. timeout_ms=0 returns a snapshot, maximum/default 60000 ms. Timeout does not cancel targets. Returns {timed_out, completed_run_ids, attention_run_ids, runs}, with each run's phase/status/output/reason and truncation flags (output 4096 bytes, reason 1024 bytes). The legacy run_id-only call returns Done/Error upon completion, and a snapshot on timeout or required user input. Self, sibling, unrelated and unknown IDs fail before waiting. Caller cancellation interrupts immediately.",
                json!({
                    "run_id": run_id(),
