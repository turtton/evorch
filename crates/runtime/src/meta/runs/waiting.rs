...
pub(in crate::meta) const MAX_WAIT_MS: u64 = 60_000;
...
impl WaitArgs {
    fn validate(self) -> Result<WaitRequest, String> {
        ...
        if timeout_ms > MAX_WAIT_MS {
            return Err("timeout_ms must be between 0 and 60000".into());
        }
        let runs = runs
            .iter()
...
pub(in crate::meta) async fn wait(
    ...
    let result = observe(
        ...
        state.caller_run_id(),
        &request,
        state.channels.cancel_rx.clone(),
    )
    .await;
...
async fn observe(
    ...
    if satisfied(&initial, request.mode) || needs_attention(&initial) || request.timeout.is_zero() {
        return Ok(response(initial, request.mode));
    }
...
fn response(runs: Vec<Value>, mode: WaitMode) -> Value {
    json!({
        "timed_out": !satisfied(&runs, mode) && !needs_attention(&runs),
