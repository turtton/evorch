//! Intent Gate セクションのレンダラ (issue #49 / AC8)。
//!
//! Orchestrator がユーザーの現在のメッセージを分類するためのゲート文を
//! 決定論的にレンダリングする。内容は keyTriggers ブロック (AC9) を
//! マーク済みセクションとして埋め込む。
...
/// マーカー直前までの固定本文。
const GATE_BODY: &str = r#"## Intent Gate
...
///
/// 固定本文の末尾に、[`render_key_triggers`] の出力をマーク済みセクション
/// として埋め込む。出力に末尾の改行は含まない。
pub fn render_intent_gate(triggers: &[TriggerSource]) -> String {
    let mut rendered = String::from(GATE_BODY);
