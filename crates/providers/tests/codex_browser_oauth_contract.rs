...
use providers::provider::codex::oauth::{
    BrowserAuthClient, BrowserAuthError, CallbackServer, CODEX_CLIENT_ID, CODEX_SCOPE, PkcePair,
};
...
fn get(uri: &str, query: &str) -> String {
    ...
    write!(stream, "GET /auth/callback?{query} HTTP/1.1\r\nHost: localhost\r\nConnection: close\r\n\r\n").expect("request");
...
fn authorize_url_contains_pkce_state_scope_and_simplified_flow() {
    ...
    assert_eq!(params["code_challenge"], PkcePair::challenge_for(request.code_verifier()));
    ...
    assert_ne!(client.begin(&server).expect("second begin").state, request.state);
}
...
fn callback_server_accepts_matching_state_and_returns_code() {
    ...
    // When
    let code = server.wait_for_code("expected", Duration::from_secs(2)).expect("code");
    // Then
    assert_eq!(code, "a+b");
...
fn bind_ports_falls_back_to_second_port_when_first_busy() {
    ...
    assert!(matches!(CallbackServer::bind_ports(&[first]), Err(BrowserAuthError::CallbackPortBusy)));
}
...
async fn complete_exchanges_code_with_browser_redirect_uri() {
    ...
    let request = client.begin(&server).expect("begin");
    let redirect = server.redirect_uri().replace(':', "%3A").replace('/', "%2F");
    Mock::given(method("POST"))
        .and(path("/oauth/token"))
        .and(body_string_contains(format!("redirect_uri={redirect}")))
        .and(body_string_contains(format!("code_verifier={}", request.code_verifier())))
    ...
    // When
    let bundle = client.complete(request, "authorization-code").await.expect("exchange");
    // Then
