//! LLM provider クライアントを統一的に扱うための抽象を提供します。

pub mod auth;
pub mod client;
pub(crate) mod dedup;
pub mod error;
pub mod http;
pub mod message;
mod models;
pub(crate) mod observe;
pub mod provider;
pub mod retry;
pub mod sse;
pub mod stream;
pub mod wire;

pub use auth::ProviderAuth;
pub use client::ProviderClient;
pub use error::ProviderError;
pub use message::{
    ChatRequest, ChatResponse, ContentBlock, FinishReason, Message, ObservationContext,
    ProviderCapabilities, ReasoningEffort, Role, ServiceTier, ToolResultContent, ToolSpec, Usage,
};
pub use models::{CODEX_MODELS_CLIENT_VERSION, CodexModelInfo, list_codex_models, list_models};
pub use retry::RetryPolicy;
pub use stream::{DeltaStream, StreamAccumulator, StreamEvent};
