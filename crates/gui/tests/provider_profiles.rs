...
fn seed_lists_all_profiles_sorted() {
    ...
    for name in ["z", "a", "m"] {
        config.providers.insert(name.into(), ProviderProfileConfig::default());
    }
    // When
    let model = ProviderSettingsModel::seed_from_config(&config);
    // Then
    assert_eq!(model.profiles.iter().map(|p| p.name.as_str()).collect::<Vec<_>>(), ["a", "m", "z"]);
...
fn add_codex_editor_defaults_account_to_profile_name() {
    ...
    // Then
    let Some(ProfileEditor::Codex(editor)) = &model.editor else { panic!("Codex editor") };
...
fn edit_existing_openai_profile_prefills_form() {
    // Given
    let mut config = Config::default();
    config.providers.insert("work".into(), ProviderProfileConfig {
        provider_type: ProviderTypeConfig::OpenAiCompatible,
        base_url: "https://example.com/v1".into(), models: vec!["model".into()],
        default_model: "model".into(), ..Default::default()
    });
    let mut model = ProviderSettingsModel::seed_from_config(&config);
    // When
    model.edit("work");
    // Then
    let Some(ProfileEditor::OpenAiCompatible(editor)) = &model.editor else { panic!("OpenAI editor") };
...
fn delete_removes_profile_and_refreshes_list() {
    ...
    config::save_codex_provider(&path, &config::CodexProviderInput {
        name: "work".into(), account: "work".into(), models: vec![], default_model: String::new()
    }).unwrap();
    // When
    config::delete_provider(&path, "work").unwrap();
    let config = Config::load(&config::LoadOptions {
        project_dir: Some(tmp.path().into()), user_config_dir: Some(tmp.path().join("empty")),
        read_env: false, ..Default::default()
    }).unwrap();
    // Then
    assert!(ProviderSettingsModel::seed_from_config(&config).profiles.is_empty());
}
