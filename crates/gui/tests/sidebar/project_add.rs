...
fn add_project_expands_tilde_before_dispatch() {
    ...
    let mut harness = Harness::builder().with_size(egui::vec2(1000.0, 700.0)).build_ui_state(
        |ui, state: &mut WorkbenchState<MockSource>| state.ui(ui, &mut eframe::Frame::_new_kittest()),
        workbench,
    );
    // When: the operator types a tilde path and presses Add project.
    harness.get_by_label("Project path (~ allowed)").type_text("~/repo");
    harness.run();
    harness.get_by_label("Add project").click();
    harness.run();
    // Then: the registered root is the canonical home-relative path.
    assert_eq!(harness.state().sidebar().projects[0].repo_root, repo.canonicalize().expect("canonical repo"));
}
...
fn project_row_shows_repo_root_path() {
    ...
    let mut harness = HeadlessWorkbench::new(state(MockSource::default(), sidebar), [1000.0, 700.0]);
...
fn browse_button_dispatches_picker_and_adds_selected_folder() {
    ...
    assert_eq!(harness.state().sidebar().projects[0].repo_root, temp.path().canonicalize().expect("root"));
    assert_eq!(workspace_ui::load_sidebar(&save).expect("saved sidebar").projects.len(), 1);
}
...
fn picker_error_surfaces_in_sidebar() {
    ...
    let workbench = state(MockSource::default(), SidebarState::default())
        .with_folder_picker(Arc::new(ScriptedFolderPicker(Err("portal unavailable".into()))));
    let mut harness = HeadlessWorkbench::new(workbench, [1000.0, 700.0]);
    harness.run();
...
fn allowed_directories_section_hidden_when_empty_and_collapsed_when_present() {
    ...
    let mut harness = HeadlessWorkbench::new(state(MockSource::default(), sidebar_with_project(temp.path())), [1000.0, 700.0]);
    harness.run();
    assert!(!harness.has_label("Allowed directories"));
    assert!(!harness.has_label("Allowed directories (0)"));
    harness.state_mut().add_allowed_directory(allowed.path()).expect("allowed");
    harness.run();
    let path = allowed.path().canonicalize().expect("canonical path").display().to_string();
