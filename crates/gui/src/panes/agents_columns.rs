...
pub(crate) fn fit_columns(natural: &[f32], spacing: f32, available: f32) -> Vec<f32> {
    ...
    let total = total_with_spacing(&widths);
    if total > available {
        let gap_total = spacing * (count.saturating_sub(1) as f32);
        let content = (available - gap_total).max(0.0);
...
mod tests {
    ...
    #[test]
    fn fit_never_returns_negative_widths_when_available_is_below_gap_total() {
        // Given: 9 columns but available width smaller than the gap total alone.
        let widths = fit_columns(&[160.0; 9], 4.0, 10.0);

        // Then: every width is non-negative (cells collapse to zero, no UB-sized widgets).
        assert_eq!(widths.len(), 9);
        assert!(widths.iter().all(|w| *w == 0.0), "widths: {widths:?}");
    }
}
