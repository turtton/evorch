...
pub fn goal_pane(
    ...
    pane_root(ui, "Goal", |ui| {
        ui.label(h4("Goal text"));
        ui.add(
        ...
        let submit = if has_active_thread {
            primary_button(ui, "Submit").clicked()
        } else {
            ui.add_enabled(false, egui::Button::new("Submit")).clicked()
        };

        if let Some(goal_id) = &goal.last_accepted {
            ui.label(egui::RichText::new(format!("accepted: {goal_id}")).color(SUCCESS));
        }

        if let Some(state) = status.state {
            let color = match state {
                GoalState::Blocked => ERROR_FG,
                GoalState::Paused => WARNING_FG,
                GoalState::Active | GoalState::Complete | GoalState::Cancelled => TEXT,
            };
            ui.label(egui::RichText::new(format!("state: {}", state_label(state))).color(color));
        }
        ...
        for rejection in &status.last_rejections {
            ui.label(egui::RichText::new(format!("rejected: {rejection}")).color(ERROR_FG));
        }
        for (step, ok) in &status.closeout {
            let color = if *ok { SUCCESS } else { ERROR_FG };
            ui.label(
