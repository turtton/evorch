...
use storage::projection::{ReconcileSummary, SessionSnapshot, reconcile, restore_session, restore_sessions};
use storage::repo::{event, session, task};
use storage::{Database, HardLimits, StorageConfig};
use tempfile::TempDir;

fn open(temp: &TempDir) -> Connection { let path = temp.path().join("resume.db"); drop(Database::open(&StorageConfig { db_path: path.clone(), ..StorageConfig::default() }).expect("database must open")); Connection::open(path).expect("database must reopen") }
...
fn interrupted_session_restores_pending_output_and_open_tool_call() {
    /* Given: 応答・推論・未完了ツールを持つセッション */ let temp = TempDir::new().unwrap(); let conn = open(&temp); append(&conn, Some("s1"), &event(started(), 1)); append(&conn, Some("s1"), &event(MessageEvent::MessageDelta { delta: "Hello".into() }, 2)); append(&conn, Some("s1"), &event(MessageEvent::ReasoningDelta { delta: "think".into() }, 3)); append(&conn, Some("s1"), &event(ToolEvent::ToolStarted { tool_name: "tool".into(), call_id: "c1".into() }, 4));
    /* When: セッションを復元する */ let actual = restore_session(&conn, "s1").unwrap();
...
fn delegated_session_restores_target() {
    /* Given: 委譲イベント */ let temp = TempDir::new().unwrap(); let conn = open(&temp); append(&conn, Some("s1"), &event(LifecycleEvent::Delegated { session_id: "s1".into(), target: "agent-a".into() }, 1));
    /* When: 復元する */ let actual = restore_session(&conn, "s1").unwrap();
...
fn completed_session_restores_terminal_status() {
    /* Given: 開始後に完了したセッション */ let temp = TempDir::new().unwrap(); let conn = open(&temp); append(&conn, Some("s1"), &event(started(), 1)); append(&conn, Some("s1"), &event(LifecycleEvent::Completed { session_id: "s1".into() }, 2));
    /* When: 復元する */ let actual = restore_session(&conn, "s1").unwrap();
...
fn failed_session_restores_reason() {
    /* Given: 失敗イベント */ let temp = TempDir::new().unwrap(); let conn = open(&temp); append(&conn, Some("s1"), &event(LifecycleEvent::Failed { session_id: "s1".into(), reason: "boom".into() }, 1));
    /* When: 復元する */ let actual = restore_session(&conn, "s1").unwrap();
...
fn unattributed_message_is_skipped_by_restore_and_reconcile() {
    /* Given: 帰属のないメッセージ */ let temp = TempDir::new().unwrap(); let conn = open(&temp); append(&conn, None, &event(MessageEvent::MessageDelta { delta: "orphan".into() }, 1));
    /* When: 復元して再調整する */ let restored = restore_sessions(&conn).unwrap(); let summary = reconcile(&conn).unwrap();
    /* Then: セッション行を作らない */ assert_eq!(restored, Vec::<SessionSnapshot>::new()); assert_eq!(summary, ReconcileSummary { sessions_upserted: 0, tasks_upserted: 0 }); assert_eq!(session::get(&conn, "s1").unwrap(), None);
}

#[test]
fn replay_uses_stored_id_order_instead_of_wall_clock_order() {
    /* Given: 壁時計と ID 順が逆のイベント */ let temp = TempDir::new().unwrap(); let conn = open(&temp); append(&conn, Some("s1"), &event(started(), 20)); append(&conn, Some("s1"), &event(LifecycleEvent::Failed { session_id: "s1".into(), reason: "last-id".into() }, 10));
    /* When: 復元する */ let actual = restore_session(&conn, "s1").unwrap();
...
fn completed_tool_call_is_removed_from_open_calls() {
    /* Given: 開始・完了したツール呼び出し */ let temp = TempDir::new().unwrap(); let conn = open(&temp); append(&conn, Some("s1"), &event(started(), 1)); append(&conn, Some("s1"), &event(ToolEvent::ToolStarted { tool_name: "tool".into(), call_id: "c1".into() }, 2)); append(&conn, Some("s1"), &event(ToolEvent::ToolCompleted { tool_name: "tool".into(), call_id: "c1".into(), is_error: false }, 3));
    /* When: 復元する */ let actual = restore_session(&conn, "s1").unwrap();
...
fn reconcile_is_idempotent_and_reports_stable_counts() {
    /* Given: 完了済みタスクのログ */ let temp = TempDir::new().unwrap(); let conn = open(&temp); append(&conn, Some("s1"), &event(started(), 1)); append(&conn, Some("s1"), &event(LifecycleEvent::BackgroundTaskStarted { task_id: "t1".into() }, 2)); append(&conn, Some("s1"), &event(LifecycleEvent::BackgroundTaskCompleted { task_id: "t1".into() }, 3));
    /* When: 二度再調整する */ let first = reconcile(&conn).unwrap(); let first_session = session::get(&conn, "s1").unwrap(); let first_task = task::get(&conn, "t1").unwrap(); let second = reconcile(&conn).unwrap();
    /* Then: 件数と行が変わらない */ assert_eq!(second, first); assert_eq!(session::get(&conn, "s1").unwrap(), first_session); assert_eq!(task::get(&conn, "t1").unwrap(), first_task);
}
}
