...
mod tests {
    ...
    #[test] fn tool_completed_closes_call() { /* Given: 開いている呼び出し */ let mut state = apply(ToolEvent::ToolStarted { tool_name: "x".into(), call_id: "c".into() }, Some("s1")); /* When: 完了を適用する */ apply_event(&mut state, &stored(ToolEvent::ToolCompleted { tool_name: "x".into(), call_id: "c".into(), is_error: false, detail: None }, Some("s1"))); /* Then: 未完了一覧から除く */ assert_eq!(session(&state).map(|v| v.open_tool_calls.as_slice()), Some([].as_slice())); }
