...
pub(crate) fn apply_event(state: &mut ProjectionState, stored: &StoredEvent) {
    match &stored.event.kind {
        ...
        // セッション射影は変更しない。
            EventKind::Orchestrator(_) | EventKind::Diagnostic(_) | EventKind::Ownership(_) => {}
    }
}
...
mod tests {
    ...
    #[test] fn tool_started_opens_call() { /* Given/When: ツール開始を適用する */ let state = apply(ToolEvent::ToolStarted { tool_name: "x".into(), call_id: "c".into(), run_id: None, input: None }, Some("s1")); /* Then: 未完了呼び出しへ追加する */ assert_eq!(session(&state).map(|v| v.open_tool_calls.clone()), Some(vec![("x".into(), "c".into())])); }
    #[test] fn tool_completed_closes_call() { /* Given: 開いている呼び出し */ let mut state = apply(ToolEvent::ToolStarted { tool_name: "x".into(), call_id: "c".into(), run_id: None, input: None }, Some("s1")); /* When: 完了を適用する */ apply_event(&mut state, &stored(ToolEvent::ToolCompleted { tool_name: "x".into(), call_id: "c".into(), is_error: false, detail: None, output: None, run_id: None }, Some("s1"))); /* Then: 未完了一覧から除く */ assert_eq!(session(&state).map(|v| v.open_tool_calls.as_slice()), Some([].as_slice())); }
    #[test] fn approval_requested_does_not_mutate_open_calls() { /* Given: 開いている呼び出し */ let mut state = apply(ToolEvent::ToolStarted { tool_name: "x".into(), call_id: "c".into(), run_id: None, input: None }, Some("s1")); /* When: 承認要求を適用する */ apply_event(&mut state, &stored(ToolEvent::ApprovalRequested { tool_name: "y".into(), call_id: "d".into() }, Some("s1"))); /* Then: 未完了一覧を保持する */ assert_eq!(session(&state).map(|v| v.open_tool_calls.as_slice()), Some([("x".into(), "c".into())].as_slice())); }
    #[test] fn approval_resolved_does_not_mutate_open_calls() { /* Given: 開いている呼び出し */ let mut state = apply(ToolEvent::ToolStarted { tool_name: "x".into(), call_id: "c".into(), run_id: None, input: None }, Some("s1")); /* When: 承認応答を適用する */ apply_event(&mut state, &stored(ToolEvent::ApprovalResolved { call_id: "c".into(), approved: true }, Some("s1"))); /* Then: 未完了一覧を保持する */ assert_eq!(session(&state).map(|v| v.open_tool_calls.as_slice()), Some([("x".into(), "c".into())].as_slice())); }
    #[test] fn execution_denied_does_not_mutate_open_calls() { /* Given: 開いている呼び出し */ let mut state = apply(ToolEvent::ToolStarted { tool_name: "x".into(), call_id: "c".into(), run_id: None, input: None }, Some("s1")); /* When: 実行拒否を適用する */ apply_event(&mut state, &stored(ToolEvent::ExecutionDenied { tool_name: "y".into(), call_id: "d".into(), reason: "policy".into() }, Some("s1"))); /* Then: 未完了一覧を保持する */ assert_eq!(session(&state).map(|v| v.open_tool_calls.as_slice()), Some([("x".into(), "c".into())].as_slice())); }
